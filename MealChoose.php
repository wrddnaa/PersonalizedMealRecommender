        ...
        <div class="main-content">
            ...
            <div class="page-content">
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0">Meal List Recommender</h4>
                            </div>
                        </div>
                    </div>
                    ...
                    $nuts=$row["nuts_status"];
                    $seafood=$row["seafood_status"];
                    $lactose=$row["dairyProduct_status"];
                    $veg = $row['vegetable_id']; 
                    $prot = $row['protein_id'];
                    $carb = $row['carb_id'];
                    ...
                    meal.ingredients, meal.mealName, meal.type, meal.pic, preferences.perihal 
                        ...
                        JOIN carb_meal ON carb_meal.meal_id = meal.id 
                        JOIN disease_meal ON disease_meal.meal_id = meal.id AND disease_meal.disease_id=$disease
                        WHERE 
                         EXISTS (
                          SELECT 1 FROM vegetable_meal WHERE vegetable_meal.meal_id = meal.id AND vegetable_meal.vegetable_id IN ($veg)
                        )
                        AND EXISTS (
                          SELECT 1 FROM protein_meal WHERE protein_meal.meal_id = meal.id AND protein_meal.protein_id IN ($prot)
                         )
                         AND EXISTS (
                          SELECT 1 FROM carb_meal WHERE carb_meal.meal_id = meal.id AND carb_meal.carb_id IN ($carb)
                         )
                        GROUP BY meal.id 
                         HAVING meal.preferences_id = $preference 
                         AND meal.nuts_status =$nuts
                         AND meal.seafood_status =$seafood
                         AND meal.dairyProduct_status = $lactose";
                       ...
                       while ($row = $result->fetch_assoc()) {
                            ...
                            echo "
                                    <div class='card'>
                                        <div class='row g-0'>
                                        <div class='col-md-4'>
                                                <a href='viewMeal.php?id=$id'>
                                                <img class='rounded-start img-fluid h-100 object-cover' src='../MealPic/$pic' alt='Card imagestyle='max-width: 400px; height: auto;'>
                                                </a>
                                            </div>
                                            <div class='col-md-8'>
                                                ...
                                                <div class='card-body'>
                                                    ...
                                                    <center><a href='viewMeal.php?id=$id'>
                                                        <button type='button' class='btn btn-primary waves-effect waves-light'>View More</button>
                                                    </a></center>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class='row g-0'>
                                            <div class='col-md-8'>
                                                ...
                                                <div class='card-body'>
                                                    ...
                                                    <p class='card-text mb-2'><b>";
                                                    $query4="SELECT lkp_carb.perihal as carb FROM carb_meal JOIN lkp_carb ON lkp_carb.id=carb_meal.id WHERE carb_meal.meal_id=$id";
                                                    $result4=$conn->query($query4);
                                                    if($result4->num_rows>0){
                                                      while ($row4 = $result4->fetch_assoc()) {
                                                        $carb=$row4['carb'];
                                        ...
                                        <div class='modal-dialog'>
                                            <div class='modal-content'>
                                                <div class='modal-header'>
                                                    <h5 class='modal-title' id='myModalLabel'>Ingredients</h5>
                                                    <button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'> </button>
                                                </div>
                                                <div class='modal-body'>
                                                    <p class='card-text mb-2'>Ingredients:<br>$ingredients</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class='alert alert-danger' role='alert'>
                            <strong>We're sorry, there is no meal recommended. Please select more ingredients when choosing your ingredients preferences.</strong> 
                        </div>
                        ";
                     }
                }
                        ...
                        echo "
                        <div class='alert alert-danger' role='alert'>
                            <strong>Please create your meal in Dashboard in order to see your own personalized meal</strong> 
                        </div>
                        ";
                     }
                    ?>
